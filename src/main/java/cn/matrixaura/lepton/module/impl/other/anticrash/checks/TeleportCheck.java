...
public class TeleportCheck extends CrashCheck {
    ...
    public boolean handle(Object packet) {
        if (PacketUtils.isPacketInstanceof(packet, "net/minecraft/network/play/server/S08PacketPlayerPosLook")) {
            double x = (Double) ReflectionUtils.invokeMethod(packet.getClass(), packet, Mappings.seargeToNotchMethod("func_148932_c"));
            double y = (Double) ReflectionUtils.invokeMethod(packet.getClass(), packet, Mappings.seargeToNotchMethod("func_148928_d"));
            double z = (Double) ReflectionUtils.invokeMethod(packet.getClass(), packet, Mappings.seargeToNotchMethod("func_148933_e"));
