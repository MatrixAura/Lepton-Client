...
public class Mappings {
    ...
    public static void readMappings(MinecraftVersion mcVer) {
        ...
        for (String line : content.split("\n")) {
            ...
            if (line.startsWith("CL: ")) { // class name
                obfClassNames.put(parts[0], parts[1]);
                classNames.put(parts[1], parts[0]);
    ...
    private static void readCsv(MinecraftVersion mcVer) {
        String content = read(mcVer.getFields());
        if (content == null || content.isEmpty())
    ...
    private static String readSrgFile(MinecraftVersion mc) {
        return read(mc.getSrg());
    }

    private static String read(String x) {
        StringBuilder builder = new StringBuilder();

        InputStream is = null;
        try {
            is = Mappings.class.getResourceAsStream(x);
